...
def are_they_equal(array_a, array_b):
    ...
    for index in range(len(array_a)):
        if array_a[index] != array_b[index]:
            index_mismatch.append(index)
            value_miss_array_a.append(array_a[index])
            value_miss_array_b.append(array_b[index])

    for index in range(len(value_miss_array_b)):
        if not value_miss_array_b[index] in value_miss_array_a:
            return False

    return True


def are_they_equal_2(array_a, array_b):
    array_a.sort()
    array_b.sort()
    print(array_a)
    print(array_b)
