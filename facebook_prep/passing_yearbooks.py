...
themselves or to another student), and then they'll pass it to student arr[i-1]. It's possible that arr[i-1] = i for
any given i, in which case student i will pass their yearbook back to themselves. Once a student has received their
own yearbook back, they will hold on to it and no longer participate in the passing process.
...
def find_signature_counts(arr: List[int]) -> List[int]:
    # everybody signs their yearbook
    cached_signatures = {}
    signatures = [0] * len(arr)                                   #  0, 1, 2, 3
    for index in range(len(arr)):                                 # [3, 2, 4, 1, 4, 5]
        # check for pass condition
        passed_to_index = None
        while True:
            if passed_to_index is None:
                passed_to_index = index
            elif passed_to_index == index:
                break

            if passed_to_index in cached_signatures:
                print('Cache Hit')
                signatures[index] = cached_signatures[passed_to_index]
                break

            signatures[index] += 1
            passed_to_index = arr[passed_to_index] - 1

        cached_signatures[index] = signatures[index]
...
def main():
    ...
    arr_3 = [3, 2, 4, 1]
    expected_3 = [3, 1, 3, 3]
    output_3 = find_signature_counts(arr_3)
