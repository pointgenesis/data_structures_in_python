...
Signature
int number_of_ways(int[] arr, int k)
...
arr = [1, 5, 3, 3, 3]
output = 4
There's one valid pair 1+5, and three different valid pairs 3+3 (the 3rd and 4th elements, 3rd and 5th elements, and
4th and 5th elements).
"""


def number_of_ways(arr, k) -> int:
    total = 0
    for index, value in enumerate(arr):                 # [1, 2, 3, 4, 3] | k = 6
        left_index = index
        while left_index + 1 < len(arr):
            left_index += 1
            if arr[index] + arr[left_index] == k:
                total += 1
    return total
