...
def count_distinct_triangles(arrays: List[List[int]]):
    array_keys = {}
    for array in arrays:
        ...
        key = f"{my_list[0]}:{my_list[1]}:{my_list[2]}"
        array_keys[key] = my_list

    count = 0
    for key, value in array_keys.items():
        count += 1
...
def main():
    ...
    expected_1 = 3
    output_1 = count_distinct_triangles(arr_1)
    check(expected_1, output_1)

    arr_2 = [(3, 4, 5), (8, 8, 9), (7, 7, 7)]
    expected_2 = 3
    output_2 = count_distinct_triangles(arr_2)
